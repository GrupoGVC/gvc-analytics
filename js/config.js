...
function pctColor(p) {
  return p >= 70 ? "var(--ok)" : p >= 40 ? "var(--warn)" : "var(--bad)";
}
function pctBadge(p) {
  const c = p >= 70 ? "bg" : p >= 40 ? "bo" : "br";
  return `<span class="badge ${c}">${fmtPct(p)}</span>`;
}
...
function mapProduto(m) {
  ...
  if (u.includes("CONSULTORIA")) return "Consultoria";
  return "Gerenciamento";
}
function mapPorte(v) {
  return (v || 0) > 5000 ? "LQ" : "SQ";
}
function isOeste(e) {
  return (e || "").toLowerCase().includes("oeste");
}
...
function monthsInRange(from7, to7) {
  const r = [];
  let [y, m] = from7.split("-").map(Number);
  const [ty, tm] = to7.split("-").map(Number);
...
function getMetaTime(from, to) {
  return (
    META_TIME_MES *
    monthsInRange(from.substring(0, 7), to.substring(0, 7)).length
  );
}
function noDataHtml(msg) {
  return `<div style="text-align:center;padding:34px 20px;color:var(--text3);font-size:12.5px;letter-spacing:.01em">${msg || "Importe o arquivo Panorama CRM para visualizar os dados"}</div>`;
}
function kpiCard(label, value, sub, colorClass, progPct) {
  const prog =
    progPct != null
      ? `<div class="prog" style="margin-top:8px"><div class="prog-fill pf${colorClass || "c"}" style="width:${Math.min(100, Math.max(0, progPct || 0))}%"></div></div>`
      : "";
...
function alertaBadge(a, d) {
  const cls = a === "OK" ? "bg" : a === "ATENCAO" ? "bo" : "br";
  return `<span class="badge ${cls}">${d}d · ${a}</span>`;
}
...
function matchConsultor(raw) {
  ...
  for (const key of Object.keys(METAS_CONSULTOR)) {
    if (stripAccents(key) === rn) return key;
  }
  for (const key of Object.keys(METAS_CONSULTOR)) {
    if (stripAccents(key).split(" ")[0] === rn.split(" ")[0]) return key;
  }
  for (const key of Object.keys(METAS_CONSULTOR)) {
    if (
      rn.includes(stripAccents(key).split(" ")[0]) ||
      stripAccents(key).includes(rn.split(" ")[0])
    )
      return key;
  }
  return raw;
...
function normalizeRow(raw) {
  ...
  function get(...keys) {
    for (const k of keys) {
      ...
      for (const rk of cols) {
        if (stripAccents(rk) === needle && raw[rk] !== "" && raw[rk] != null)
          return raw[rk];
      }
    }
    for (const k of keys) {
      ...
      const found = cols.find(
        (rk) =>
          stripAccents(rk).includes(needle) &&
          raw[rk] !== "" &&
          raw[rk] != null,
      );
  ...
  const rawStageRaw =
    ...
    (status === "ganho"
      ...
      : status === "perdido"
        ...
        : "Em aberto");
  const rawStage =
    rawStageRaw === "Forcast" || rawStageRaw === "forcast"
      ? "Forcast"
      : rawStageRaw;
  const rawValor = get(
    "Valor",
  ...
  const dataStr =
    ddFinal && !isNaN(ddFinal) ? ddFinal.toISOString().substring(0, 10) : "";
  ...
  if (rawDiasEstagio !== "" && rawDiasEstagio != null) {
    ...
    else if (dcriado && !isNaN(dcriado))
      dias = Math.max(0, Math.floor((TODAY - dcriado) / (1000 * 60 * 60 * 24)));
  } else if (dcriado && !isNaN(dcriado)) {
    dias = Math.max(0, Math.floor((TODAY - dcriado) / (1000 * 60 * 60 * 24)));
  } else if (dd && !isNaN(dd) && status === "pipeline") {
    dias = Math.max(0, Math.floor((TODAY - dd) / (1000 * 60 * 60 * 24)));
  }
  const alerta = dias <= 7 ? "OK" : dias <= 30 ? "ATENCAO" : "CRITICO";
  const tipoVendaRaw = get("Tipo de Venda", "Tipo Venda", "TipoVenda");
  const modeloPropostaRaw = get(
    "Modelo da Proposta",
    "Modelo Proposta",
    "Tipo de Proposta",
    "modelo",
  ...
  let produto;
  if (tipoVendaRaw) {
    produto = tipoVendaRaw;
  } else if (modeloPropostaRaw) {
    produto = mapProduto(modeloPropostaRaw);
  } else {
  ...
  const consultorRaw = get(
    "Responsável",
    "Responsavel",
    "Responsável da Última venda",
    "Responsável da Ultima venda",
    "Responsavel da Ultima venda",
    "Responsável da última venda",
    "Responsável da Ultima Venda",
    "Proprietário",
    "Proprietario",
  ...
  const statusAltRaw = get(
    "Situação*",
    "Situacao*",
    "Situação Alternativa",
    "Situacao Alternativa",
  );
  const meioContatoRaw = get(
    "Meio de contato",
    "Meio de Contato",
    "Meio Contato",
    "Canal de Contato",
  ...
  return {
    titulo: tituloRaw,
    cliente: clienteRaw,
    consultor: consultorNorm,
    responsavel: consultorNorm,
    empresa: empresaRaw,
    empresaVendedora: empresaRaw,
    origem: origemRaw,
    motivoPerda: motivoPerdaRaw,
    motivoDePerda: motivoPerdaRaw,
    produto,
    tipoVenda: tipoVendaRaw || produto,
    tipoNegocio: produto,
    modelo,
    etapa: rawStage,
    estagio: rawStage,
    situacao: rawSituacao,
    statusAlternativo: statusAltRaw,
    status,
    valor,
    prop,
    dias,
    ...
    monthKey,
    porte: mapPorte(valor),
    marcadores: marcadoresRaw,
    dataCriacao:
      dcriado && !isNaN(dcriado) ? dcriado.toISOString().substring(0, 10) : "",
    ultimaAtualizacao:
      dultAtual && !isNaN(dultAtual)
        ? dultAtual.toISOString().substring(0, 10)
        : "",
    meioContato: meioContatoRaw,
    cidadeColeta: cidadeColetaRaw,
  };
}
...
function handleFileUpload(file) {
  if (!file) return;
  const inp = document.getElementById("crm-file-input");
  if (inp) inp.value = "";
  ...
  if (name.endsWith(".csv")) {
    ...
    r.onerror = () =>
      setImportStatus("Erro ao ler o arquivo CSV", "var(--bad)");
    r.onload = (e) => {
      try {
        ...
        if (/Ã[§£¢¡]|â€/.test(content)) {
          ...
          r2.onerror = () =>
            setImportStatus(
              "Erro ao ler o arquivo CSV (encoding)",
              "var(--bad)",
            );
          r2.onload = (e2) => {
            try {
              parseCSV(e2.target.result, file.name);
            } catch (err) {
              setImportStatus("Erro CSV: " + err.message, "var(--bad)");
            }
          };
    ...
    const r = new FileReader();
    r.onerror = () =>
      setImportStatus("Erro ao ler o arquivo XLSX", "var(--bad)");
    r.onload = (e) => {
      try {
        parseXLSX(e.target.result, file.name);
      } catch (err) {
        setImportStatus("Erro XLSX: " + err.message, "var(--bad)");
        console.error(err);
      }
    };
...
function parseCSV(content, filename) {
  try {
    ...
    processRawRows(r.data, filename);
  } catch (err) {
    setImportStatus("Erro CSV: " + err.message, "var(--bad)");
  }
}
...
function processRawRows(rows, filename) {
  try {
    if (!rows || !rows.length) {
      setImportStatus("Arquivo vazio ou sem dados", "var(--warn)");
      return;
    }
    if (rows[0]) {
      const colsEssenciais = [
        "Título",
        "Cliente",
        "Valor",
        "Responsável",
      ...
      const ausentes = colsEssenciais.filter(
        (ec) =>
          !rowCols.some(
            (rc) => rc === stripAccents(ec) || rc.includes(stripAccents(ec)),
          ),
      );
      if (ausentes.length) {
        console.warn("Colunas ausentes no Panorama:", ausentes);
      }
    }
    const normalized = [];
    let erros = 0;
    ...
    const valid = normalized.filter(
      (r) => r.cliente || r.consultor || r.valor > 0,
    );
    if (!valid.length) {
      const sampleCols = rows[0]
        ? Object.keys(rows[0]).slice(0, 10).join(" | ")
        : "(sem colunas)";
      setImportStatus("Nenhum registro. Colunas: " + sampleCols, "var(--bad)");
      return;
    }
    const semDataFechado = valid.filter(
      (r) => (r.status === "ganho" || r.status === "perdido") && !r.data,
    ).length;
    if (semDataFechado > 0) {
      console.warn(
        "Atenção: " +
          semDataFechado +
          " negócios ganhos/perdidos sem Data de Fechamento.",
      );
    }
    ...
    const cons = [
      ...new Set(valid.map((r) => r.consultor).filter(Boolean)),
    ].sort();
    populateConsultorFilter(cons);
    populateGestorSelect();
    const allDates = valid
      .filter((r) => r.data)
      .map((r) => r.data)
      .sort();
    if (allDates.length) {
      const [minY, minM] = allDates[0].split("-");
      const [maxY, maxM] = allDates[allDates.length - 1].split("-");
      ...
      const df = document.getElementById("dt-from");
      const dt = document.getElementById("dt-to");
      if (df) df.value = fromVal;
      if (dt) dt.value = toVal;
      const v = allDates[allDates.length - 1];
      ["data-ate", "data-ate-h"].forEach((id) => {
    ...
    const warnData =
      semDataFechado > 0 ? ` · ⚠ ${semDataFechado} sem data de fechamento` : "";
    setImportStatus(
      `✓ ${filename} — ${valid.length} reg (${nG}G · ${nP}P · ${nPl}Pipe)${errMsg}${warnData}`,
      "var(--ok)",
    );
  } catch (err) {
    setImportStatus("Erro interno: " + err.message, "var(--bad)");
    console.error(err);
  }
}
function populateEmpresaFilter(emps) {
  const c = document.getElementById("empresa-checkboxes");
  if (!c) return;
  const all = [...new Set([...EMPRESAS_PADRAO, ...emps])].sort();
  c.innerHTML = all
    .map(
      (e) =>
        `<label class="chk-label"><input type="checkbox" value="${e.replace(/"/g, "&quot;")}" onchange="applyFilters()"><span>${e}</span></label>`,
    )
    .join("");
}
function populateConsultorFilter(cons) {
  const s = document.getElementById("sel-resp");
  if (!s) return;
  const cv = s.value;
  s.innerHTML =
    '<option value="">Todos</option>' +
    cons.map((c) => `<option value="${c}">${c}</option>`).join("");
  s.value = cv;
}
function populateGestorSelect() {
  const s = document.getElementById("gestor-seller-select");
  if (!s) return;
  const cv = s.value;
  s.innerHTML =
    '<option value="">— Selecione —</option>' +
    Object.keys(METAS_CONSULTOR)
...
function saveLS() {
  try {
    localStorage.setItem("gvc_crm_data", JSON.stringify(AppState.rawData));
    localStorage.setItem("gvc_crm_meta", JSON.stringify(AppState.importMeta));
  } catch (e) {}
}
function loadLS() {
  try {
    const d = localStorage.getItem("gvc_crm_data");
    const m = localStorage.getItem("gvc_crm_meta");
    if (d && m) {
      AppState.rawData = JSON.parse(d);
      AppState.importMeta = JSON.parse(m);
      setImportStatus(
        `${AppState.importMeta.filename} · ${AppState.importMeta.importedAt}`,
        "var(--info)",
      );
      const emps = [
        ...new Set(AppState.rawData.map((r) => r.empresa).filter(Boolean)),
      ];
      populateEmpresaFilter(emps);
      const cons = [
      ...
      populateConsultorFilter(cons);
      populateGestorSelect();
      const dates = AppState.rawData
        .filter((r) => r.data)
        .map((r) => r.data)
        .sort();
      if (dates.length) {
        const v = dates[dates.length - 1];
        ["data-ate", "data-ate-h"].forEach((id) => {
          const e = document.getElementById(id);
          if (e) e.textContent = v;
        });
...
function applyFilters() {
  ...
  const checked = Array.from(
    document.querySelectorAll("#empresa-checkboxes input:checked"),
  ).map((i) => i.value);
  AppState.filters = {
    dateFrom: from,
    dateTo: to,
  ...
  if (fa) fa.style.display = active ? "flex" : "none";
  const pl = document.getElementById("period-label");
  if (pl) pl.textContent = `${from} → ${to}`;
  updateDashboard();
}
function clearFilters() {
  ...
  document
    .querySelectorAll("#empresa-checkboxes input")
...
function renderMarcadores() {
  ...
  if (no) {
    ...
    ["marc-dist-table", "marc-detail-table", "marc-consultor-table"].forEach(
      (id) => {
  ...
  if (kpisEl)
    kpisEl.innerHTML = [
      kpiCard(
        "Total Acumulado",
        fmt(totalVal),
        `${fmtN(totalDeals)} deals ganhos · Mês Atual: ${fmt(curVal)}`,
        "c",
        totalVal > 0 ? 100 : 0,
      ),
      ...mRows.map((r) =>
        kpiCard(
          r.m,
          fmt(r.val),
          `${fmtN(r.deals)} deals · Ticket: ${fmt(r.ticket)} · ${fmtPct(r.pct)} do total`,
          "",
          r.pct,
        ),
      ),
  ...
  if (detailEl)
    detailEl.innerHTML = `<table class="dt"><thead><tr>
  ...
  if (distEl) {
    const monthRows = months.map((mk) => {
      ...
      const byM = MARCADORES.map((marc) => {
        const mm = mg.filter((r) => (r.marcadores || "") === marc);
        return { val: mm.reduce((s, r) => s + r.valor, 0), deals: mm.length };
      });
      return { label, mv, byM, deals: mg.length };
    });
    const totByM = MARCADORES.map((_, i) => ({
      val: monthRows.reduce((s, r) => s + r.byM[i].val, 0),
      deals: monthRows.reduce((s, r) => s + r.byM[i].deals, 0),
    }));
  ...
  if (typeof Chart !== "undefined") {
    ...
    if (cEl) {
      ...
      const allG2026 = AppState.rawData.filter(
        (r) =>
          r.status === "ganho" &&
          r.monthKey &&
      ...
      const datasets = MARCADORES.map((marc, i) => ({
        ...
        data: fixedMonths.map((mk) =>
          allG2026
            .filter((r) => (r.marcadores || "") === marc && r.monthKey === mk)
  ...
  if (consEl) {
    ...
    const cRows = sellers
      .map((sel) => {
        ...
        const cols = MARCADORES.map((marc) => {
          const mm = sg.filter((r) => (r.marcadores || "") === marc);
          return { val: mm.reduce((s, r) => s + r.valor, 0), deals: mm.length };
        });
        return { sel, totalSel, deals: sg.length, cols };
      })
      .sort((a, b) => b.totalSel - a.totalSel);
    const totCols = MARCADORES.map((_, i) => ({
      val: cRows.reduce((s, r) => s + r.cols[i].val, 0),
      deals: cRows.reduce((s, r) => s + r.cols[i].deals, 0),
    }));
    ...
    <tr style="border-top:2px solid var(--border-soft);font-weight:700">
      ...
      : noDataHtml("Sem dados no período");
  }
}
