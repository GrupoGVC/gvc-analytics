...
function renderConsolidado(){
  ...
  const fcDeals=pipeline.filter(r=>{const e=(r.etapa||'').toLowerCase();return e.includes('forcast')||e.includes('forecast');});
  ...
  const vendMesAtual=ganhos.filter(r=>r.monthKey===curMonthKey).reduce((s,r)=>s+r.valor,0);
  ...
  const gAtual=allRaw.filter(r=>r.status==='ganho'&&r.monthKey===curMonthKey);
  const closedMonths=monthsInRange(`${CURRENT_YEAR}-01`,`${CURRENT_YEAR}-${String(CURRENT_MONTH-1).padStart(2,'0')}`).filter(m=>m<curMonthKey);
  const gFechados=allRaw.filter(r=>r.status==='ganho'&&closedMonths.includes(r.monthKey));
  const gYTD=allRaw.filter(r=>r.status==='ganho'&&r.monthKey&&r.monthKey.startsWith(String(CURRENT_YEAR)));
  ...
  const gPrevMes=allRaw.filter(r=>r.status==='ganho'&&r.monthKey===prevMonthKey);
  const prevMonthName=MONTH_NAMES[prevMonthNum-1];
  const mcEl=document.getElementById('cons-meta-cards');
  mcEl.innerHTML=metaCardData('Mês Atual','atual',gAtual,metaMes,'#bcd6ff')+
    metaCardData(`Mês Anterior (${prevMonthName})`,'anterior',gPrevMes,metaMes,'#d9c8ff')+
    metaCardData('Meses Fechados','fechado',gFechados,metaMes*(Math.max(0,CURRENT_MONTH-1)),'#9effd4')+
    metaCardData('YTD 2026','ytd',gYTD,metaMes*CURRENT_MONTH,'#ffd79e');

  if(no){
    ['cons-mix-table','cons-origens-table','cons-funil-viz','cons-porte-viz','cons-ranking-top5','cons-ganhos-recentes','cons-motivos','cons-quarentena','cons-consultor-table'].forEach(id=>{
      const el=document.getElementById(id);if(el)el.innerHTML=noDataHtml();
    });
    document.getElementById('cons-gauge').innerHTML='';
    document.getElementById('cons-kpis').innerHTML=noDataHtml();return;
  }
  ...
  const metaTimeProd=(p)=>Object.values(METAS_CONSULTOR).reduce((s,m)=>s+(m[p]||0),0)*numMonths;
  mixEl.innerHTML=`<table class="dt"><thead><tr><th>Produto</th><th class="num">Realizado</th><th class="num">Meta</th><th class="num">% Ating.</th></tr></thead><tbody>`+
    produtos.map(p=>{
      const v=ganhos.filter(r=>r.produto===p).reduce((s,r)=>s+r.valor,0);
      const m=metaTimeProd(p);const pct=m>0?v/m*100:0;
  ...
  const oriMap={};ganhos.forEach(r=>{const o=r.origem||'(sem origem)';oriMap[o]=(oriMap[o]||0)+r.valor;});
  oriEl.innerHTML=`<table class="dt"><thead><tr><th>Origem</th><th class="num">Realizado</th><th class="num">Meta (Período)</th><th class="num">% Ating.</th><th class="num">Rep.%</th></tr></thead><tbody>`+
    METAS_ORIGEM.map(m=>{
  ...
  const ytdG=AppState.rawData.filter(r=>r.status==='ganho'&&r.monthKey&&r.monthKey.startsWith(String(CURRENT_YEAR)));
  ...
  gEl.innerHTML=`<div class="card-title" style="text-align:center">% Sobre a Meta Anual 2026</div>`+
  ...
  const allMonths2026=['2026-01','2026-02','2026-03','2026-04','2026-05','2026-06','2026-07','2026-08','2026-09','2026-10','2026-11','2026-12'];
  const staticLabels=allMonths2026.map(mk=>{const[y,m]=mk.split('-');return MONTH_NAMES[parseInt(m)-1].substring(0,3)+'/'+y.substring(2);});
  const allGanhos2026Static=AppState.rawData.filter(r=>r.status==='ganho'&&r.monthKey&&r.monthKey.startsWith('2026'));
  const vendasMesStatic=allMonths2026.map(mk=>allGanhos2026Static.filter(r=>r.monthKey===mk).reduce((s,r)=>s+r.valor,0));
  const metaMes2Static=allMonths2026.map(()=>META_TIME_MES);
  dc('chart-main');const ctxM=document.getElementById('chart-main');
  if(ctxM)new Chart(ctxM,{type:'line',data:{labels:staticLabels,datasets:[
    {label:'Vendas 2026',data:vendasMesStatic,borderColor:'#bcd6ff',backgroundColor:'rgba(188,214,255,.1)',tension:.3,fill:true,pointRadius:4,pointBackgroundColor:'#bcd6ff'},
    {label:'Meta',data:metaMes2Static,borderColor:'#ffd79e',borderDash:[5,4],backgroundColor:'transparent',tension:0,pointRadius:0}
  ]},options:{...chartDefaults(),plugins:{...chartDefaults().plugins,
    title:{display:true,text:'Vendas vs Meta 2026 — Estático, não afetado por filtros',color:'rgba(255,255,255,.45)',font:{size:9}},
    tooltip:{callbacks:{label:c=>`${c.dataset.label}: ${fmt(c.raw)}`}}}}});

  const allFd2026Static=AppState.rawData.filter(r=>r.monthKey&&r.monthKey.startsWith('2026'));
  const convMesStatic=allMonths2026.map(mk=>{const g=allFd2026Static.filter(r=>r.status==='ganho'&&r.monthKey===mk).length;const p=allFd2026Static.filter(r=>r.status==='perdido'&&r.monthKey===mk).length;return g+p>0?g/(g+p)*100:0;});
  dc('chart-conv');const ctxC=document.getElementById('chart-conv');
  if(ctxC)new Chart(ctxC,{type:'line',data:{labels:staticLabels,datasets:[{label:'Conversão % 2026',data:convMesStatic,borderColor:'#9effd4',backgroundColor:'rgba(158,255,212,.1)',tension:.3,fill:true,pointRadius:4,pointBackgroundColor:'#9effd4'}]},options:{...chartDefaults(),plugins:{...chartDefaults().plugins,
    title:{display:true,text:'Conversão 2026 — Estático, não afetado por filtros',color:'rgba(255,255,255,.45)',font:{size:9}},
    tooltip:{callbacks:{label:c=>`${fmtPct(c.raw)}`}}},scales:{...chartDefaults().scales,y:{...chartDefaults().scales.y,max:100,ticks:{...chartDefaults().scales.y.ticks,callback:v=>v+'%'}}}}});

  // [A1.5] Funil visualization — mostra qtd + valor R$ por etapa
  const fvEl=document.getElementById('cons-funil-viz');
  const etapaMap={};pipeline.forEach(r=>{const e=r.etapa||'Em aberto';if(!etapaMap[e])etapaMap[e]={count:0,valor:0};etapaMap[e].count++;etapaMap[e].valor+=(r.prop||r.valor);});
  const etapas=FUNIL_ORDER.filter(e=>etapaMap[e]).map(e=>({e,d:etapaMap[e]}));
  Object.keys(etapaMap).forEach(e=>{if(!FUNIL_ORDER.includes(e))etapas.push({e,d:etapaMap[e]});});
  const maxVal=Math.max(1,...etapas.map(x=>x.d.valor));
  // % conversão aproximada = ratio para próxima etapa visível
  fvEl.innerHTML=etapas.slice(0,8).map(({e,d},idx,arr)=>{
    const next=arr[idx+1];
    const convPctStr=next?fmtPct(next.d.count/Math.max(1,d.count)*100):'—';
  ...
  const lq=ganhos.filter(r=>r.porte==='LQ'),sq=ganhos.filter(r=>r.porte==='SQ');
  const lqV=lq.reduce((s,r)=>s+r.valor,0),sqV=sq.reduce((s,r)=>s+r.valor,0),totV=lqV+sqV;
  const totD=lq.length+sq.length;
  const lqDpct=totD>0?lq.length/totD*100:0,lqVpct=totV>0?lqV/totV*100:0;
  ...
  const ytdGPorte=allRawForPorte.filter(r=>r.status==='ganho'&&r.monthKey&&r.monthKey.startsWith('2026'));
  const ytdPorteV=ytdGPorte.reduce((s,r)=>s+r.valor,0);
  const ytdPorteN=ytdGPorte.length;
  const mesGPorte=allRawForPorte.filter(r=>r.status==='ganho'&&r.monthKey===curMonthKey);
  ...
  const top5=Object.keys(METAS_CONSULTOR).map((s,i)=>{
    const sg=ganhos.filter(r=>r.consultor===s);
    const v=sg.reduce((a,r)=>a+r.valor,0);
    const m=METAS_CONSULTOR[s].total*numMonths;
    return {s,v,m,pct:m>0?v/m*100:0,col:SELLER_COLORS[i%SELLER_COLORS.length]};
  }).sort((a,b)=>b.v-a.v).slice(0,5);
  const medals=['🥇','🥈','🥉','4','5'];
  r5El.innerHTML=top5.map((r,i)=>`
  ...
  const recentes=ganhos.filter(r=>r.data).sort((a,b)=>b.data.localeCompare(a.data)).slice(0,10);
  if(recentes.length){
    grEl.innerHTML=`<table class="dt"><thead><tr><th>Cliente</th><th>Produto</th><th>Consultor</th><th class="num">Valor</th><th>Origem</th><th>Porte</th><th>Data</th></tr></thead><tbody>`+
      recentes.map(r=>`<tr>
  ...
  const motivoMap={};perdidos.forEach(r=>{const m=r.motivoPerda||'(não informado)';if(!motivoMap[m])motivoMap[m]={count:0,valor:0};motivoMap[m].count++;motivoMap[m].valor+=r.valor;});
  const topM=Object.entries(motivoMap).sort((a,b)=>b[1].count-a[1].count).slice(0,6);
  if(topM.length){
    const mTotQtd=topM.reduce((s,[,d])=>s+d.count,0);
    const mTotVal=topM.reduce((s,[,d])=>s+d.valor,0);
    motEl.innerHTML=`<table class="dt"><thead><tr><th>#</th><th>Motivo</th><th class="num">Qtd</th><th class="num">Valor Perda</th><th class="num">%</th></tr></thead><tbody>`+
      topM.map(([m,d],i)=>`<tr><td>${i+1}</td><td class="hl">${m}</td><td class="num">${d.count}</td><td class="num">${fmt(d.valor)}</td><td class="num">${fmtPct(perdidos.length>0?d.count/perdidos.length*100:0)}</td></tr>`).join('')+
  ...
  const qar=pipeline.filter(r=>r.dias>30).sort((a,b)=>b.dias-a.dias).slice(0,10);
  if(qar.length){
    const qTotVal=qar.reduce((s,r)=>s+(r.prop||r.valor),0);
    const qTotPct=qar.length>0?100:0;
    qEl.innerHTML=`<table class="dt"><thead><tr><th>Cliente</th><th>Consultor</th><th>Etapa</th><th class="num">Proposta</th><th class="num">Dias</th><th>Alerta</th></tr></thead><tbody>`+
      qar.map(r=>`<tr><td class="hl">${r.cliente||'—'}</td><td>${r.consultor||'—'}</td><td>${r.etapa||'—'}</td><td class="num">${fmt(r.prop||r.valor)}</td><td class="num" style="color:var(--bad)">${r.dias||0}d</td><td>${alertaBadge(r.alerta,r.dias)}</td></tr>`).join('')+
  ...
  if(ftEl){
    ...
    const allFC=AppState.rawData.filter(r=>{const e=(r.etapa||'').toLowerCase();return (e.includes('forcast')||e.includes('forecast'))&&r.status==='pipeline';});
    if(allFC.length){
      ftEl.innerHTML=`<table class="dt"><thead><tr>
      ...
      allFC.sort((a,b)=>(b.prop||b.valor)-(a.prop||a.valor)).map(r=>{
        const isOld=r.monthKey&&r.monthKey<curMK2;
        const rowStyle=isOld?'background:rgba(255,75,106,.07);':'';
        const alertBadge=isOld?`<span class="badge br">⚠️ Mês anterior</span>`:`<span class="badge bc">Em aberto</span>`;
  ...
  const cRows=sellers.map(s=>{
    const sg=ganhos.filter(r=>r.consultor===s),sp=perdidos.filter(r=>r.consultor===s);
    const v=sg.reduce((a,r)=>a+r.valor,0),m=METAS_CONSULTOR[s].total*numMonths;
    const pct=m>0?v/m*100:0,conv=sg.length+sp.length>0?sg.length/(sg.length+sp.length)*100:0;
    return {s,v,m,pct,g:sg.length,p:sp.length,conv,ticket:sg.length>0?v/sg.length:0};
  }).sort((a,b)=>b.v-a.v);
  const cTotV=cRows.reduce((s,r)=>s+r.v,0),cTotM=cRows.reduce((s,r)=>s+r.m,0),cTotG=cRows.reduce((s,r)=>s+r.g,0),cTotP=cRows.reduce((s,r)=>s+r.p,0);
  const cTotPct=cTotM>0?cTotV/cTotM*100:0,cTotConv=cTotG+cTotP>0?cTotG/(cTotG+cTotP)*100:0;
  cEl.innerHTML=`<table class="dt"><thead><tr><th>#</th><th>Consultor</th><th class="num">Vendas</th><th class="num">Meta</th><th class="num">% Ating.</th><th class="num">Ganhas</th><th class="num">Perdidas</th><th class="num">Conv.</th><th class="num">Ticket</th></tr></thead><tbody>`+
    cRows.map((r,i)=>`<tr><td>${i+1}</td><td class="hl">${r.s}</td><td class="num">${fmt(r.v)}</td><td class="num">${fmt(r.m)}</td><td class="num">${pctBadge(r.pct)}</td><td class="num">${r.g}</td><td class="num">${r.p}</td><td class="num">${fmtPct(r.conv)}</td><td class="num">${fmt(r.ticket)}</td></tr>`).join('')+
  ...
  document.getElementById('cons-kpis').innerHTML=[
    kpiCard('Vendas Acumuladas',fmt(totalVendas),`Meta: ${fmt(totalMeta)}`,'c',pctMeta),
    kpiCard('Meta Acumulada',fmt(totalMeta),fmtPct(pctMeta)+' atingido','gold',pctMeta),
    kpiCard('Conversão Geral',fmtPct(conversao),`${ganhos.length}G / ${total} total`,'g',conversao),
    kpiCard('Ticket Médio',fmt(ticketMedio),`${ganhos.length} negócios ganhos`,'b'),
    kpiCard('Não Conversão',fmtN(perdidos.length),`Pipeline: ${pipeline.length} deals`,'r')
  ].join('');
}
function renderRanking(){
  if(AppState.rawData.length===0){
    ...
    document.getElementById('rank-alertas').innerHTML='';return;
  }
  ...
  const rows=Object.keys(METAS_CONSULTOR).map((s,i)=>{
    ...
    const conv=sg.length+sp.length>0?sg.length/(sg.length+sp.length)*100:0;
    const ticket=sg.length>0?v/sg.length:0;
    const status=pct>=90?'🟢':pct>=70?'🟡':'🔴';
    return {pos:0,s,v,m,metaMes:METAS_CONSULTOR[s].total,pct,conv,ticket,status,col:SELLER_COLORS[i%SELLER_COLORS.length],daysLeft,ganhas:sg.length,perdidas:sp.length};
  }).sort((a,b)=>b.v-a.v).map((r,i)=>({...r,pos:i+1}));

  // [A2.1] Table — no Empresa column
  const rkTotV=rows.reduce((s,r)=>s+r.v,0),rkTotM=rows.reduce((s,r)=>s+r.m,0);
  const rkTotG=rows.reduce((s,r)=>s+r.ganhas,0),rkTotP=rows.reduce((s,r)=>s+r.perdidas,0);
  document.getElementById('rank-table').innerHTML=`<table class="dt"><thead><tr><th>Pos</th><th>Consultor</th><th class="num">Vendas Totais</th><th class="num">Meta/Mês</th><th class="num">Atingimento</th><th class="num">Conversão</th><th class="num">Ticket</th><th>Status</th></tr></thead><tbody>`+
    rows.map(r=>`<tr>
  ...
  document.getElementById('rank-gauges').innerHTML=rows.map(r=>{
  ...
  alertasEl.innerHTML=rows.map(r=>{
    let msg='',cls='green';
    if(r.pct>=90){msg=`✅ ${r.s.split(' ')[0]}: ${fmtPct(r.pct)} — No ritmo para bater a meta.`;}
    else if(r.pct>=70){
      const gap=Math.max(0,r.m-r.v);const needed=r.daysLeft>0?gap/r.daysLeft:gap;
      msg=`🟡 ${r.s.split(' ')[0]}: ${fmtPct(r.pct)} atingido. Faltam ${fmt(gap)} · Necessário ${fmt(needed)}/dia (${r.daysLeft}d restantes).`;cls='gold';
    }else{
      const gap=Math.max(0,r.m-r.v);const needed=r.daysLeft>0?gap/r.daysLeft:gap;
      msg=`🔴 ${r.s.split(' ')[0]}: apenas ${fmtPct(r.pct)} atingido. Atenção: ${r.daysLeft} dias restantes, faltam ${fmt(gap)} (${fmt(needed)}/dia).`;cls='red';
    }
...
function renderFunil(){
  if(AppState.rawData.length===0){
    ['funil-kpis','funil-table','funil-motivos','funil-pipeline'].forEach(id=>{const el=document.getElementById(id);if(el)el.innerHTML=noDataHtml();});return;
  }
  const fd=AppState.filteredData.filter(r=>r.monthKey&&r.monthKey.startsWith('2026'));
  const pipeline=fd.filter(r=>r.status==='pipeline');
  const ganhos=fd.filter(r=>r.status==='ganho');
  ...
  document.getElementById('funil-kpis').innerHTML=[
    kpiCard('Total Pipeline',fmt(totalPipe),`${pipeline.length} negócios`,'c'),
    kpiCard('Negócios Ganhos',fmt(ganhos.reduce((s,r)=>s+r.valor,0)),`${ganhos.length} deals`,'g'),
    kpiCard('Negócios Perdidos',fmtN(perdidos.length),`${fmt(perdidos.reduce((s,r)=>s+r.valor,0))} perdidos`,'r'),
    kpiCard('Total Negócios',fmtN(fd.length),`Ganhos + Perdidos + Pipeline`,'b')
  ].join('');

  // [A3.3] Forecast projection chart
  const from=AppState.filters.dateFrom,to=AppState.filters.dateTo;
  const months=monthsInRange(from.substring(0,7),to.substring(0,7));
  const labels=months.map(mk=>{const[y,m]=mk.split('-');return MONTH_NAMES[parseInt(m)-1].substring(0,3)+'/'+y.substring(2);});
  const curMK=`${CURRENT_YEAR}-${String(CURRENT_MONTH).padStart(2,'0')}`;
  const fcVal=pipeline.filter(r=>{const e=(r.etapa||'').toLowerCase();return e.includes('forcast')||e.includes('forecast');}).reduce((s,r)=>s+(r.prop||r.valor),0);
  const realizadoMes=months.map(mk=>ganhos.filter(r=>r.monthKey===mk).reduce((s,r)=>s+r.valor,0));
  const projecaoMes=months.map((mk,i)=>mk===curMK?realizadoMes[i]+fcVal:realizadoMes[i]);
  dc('chart-forecast');
  const ctxF=document.getElementById('chart-forecast');
  if(ctxF)new Chart(ctxF,{type:'line',data:{labels,datasets:[
    {label:'Realizado',data:realizadoMes,borderColor:'#9effd4',backgroundColor:'rgba(158,255,212,.1)',tension:.3,fill:true,pointRadius:4,pointBackgroundColor:'#9effd4'},
    {label:'Projeção (c/ Forecast)',data:projecaoMes,borderColor:'#bcd6ff',backgroundColor:'rgba(188,214,255,.08)',borderDash:[6,3],tension:.3,fill:false,pointRadius:4,pointBackgroundColor:'#bcd6ff'},
    {label:'Meta',data:months.map(()=>META_TIME_MES),borderColor:'#ffd79e',borderDash:[4,4],backgroundColor:'transparent',tension:0,pointRadius:0}
  ]},options:{...chartDefaults(),plugins:{...chartDefaults().plugins,tooltip:{callbacks:{
    title:items=>labels[items[0].dataIndex],
    label:ctx=>{
      const mk=months[ctx.dataIndex];const r=realizadoMes[ctx.dataIndex];
      if(ctx.dataset.label==='Realizado')return `Realizado: ${fmt(r)}`;
      if(ctx.dataset.label==='Projeção (c/ Forecast)')return [`Forecast: ${fmt(fcVal)}`,`Total Projetado: ${fmt(projecaoMes[ctx.dataIndex])}`];
      return `Meta: ${fmt(META_TIME_MES)}`;
    }
  }}}}});

  // [A3.1] Etapas do funil — with benchmark and alert
  const BENCH_PCT={Fechamento:5,Contrato:10,Forcast:15,Forecast:15,Negociação:20,Reunião:20,'Reunião Agendada':25,Agendamento:30,Conexão:50,'Novos Leads':100};
  const etapaMap={};fd.forEach(r=>{const e=r.etapa||'(sem etapa)';if(!etapaMap[e])etapaMap[e]={count:0,valor:0};etapaMap[e].count++;etapaMap[e].valor+=r.valor;});
  const etapas=Object.entries(etapaMap).sort((a,b)=>b[1].valor-a[1].valor);
  ...
  ftEl.innerHTML=`<table class="dt"><thead><tr>
    ...
    etapas.map(([e,v])=>{
      ...
      let alerta='';
      if(bench){const diff=pct-bench;alerta=diff>5?`<span class="badge bg">▲ Acima</span>`:diff<-5?`<span class="badge br">▼ Abaixo</span>`:`<span class="badge bc">✓ OK</span>`;}
    ...
    (()=>{const totVal=etapas.reduce((s,[,v])=>s+v.valor,0);const totCnt=etapas.reduce((s,[,v])=>s+v.count,0);return `<tr style="border-top:2px solid var(--border-soft);background:rgba(255,255,255,.04)"><td><strong style="color:var(--text)">TOTAL</strong></td><td class="num"><strong>${fmt(totVal)}</strong></td><td class="num"><strong>${totCnt}</strong></td><td class="num"><strong>100%</strong></td><td></td><td></td></tr>`;})()
    +`</tbody></table>`;

  // [A3.2] Motivos — with valor
  const motivoMap={};perdidos.forEach(r=>{const m=r.motivoPerda||'(não informado)';if(!motivoMap[m])motivoMap[m]={count:0,valor:0};motivoMap[m].count++;motivoMap[m].valor+=r.valor;});
  const topM=Object.entries(motivoMap).sort((a,b)=>b[1].count-a[1].count);
  ...
  if(topM.length){
    ...
    fmEl.innerHTML=`<table class="dt"><thead><tr><th>#</th><th>Motivo</th><th class="num">Qtd</th><th class="num">Valor R$</th><th class="num">%</th></tr></thead><tbody>`+
      topM.slice(0,8).map(([m,d],i)=>`<tr><td>${i+1}</td><td class="hl">${m}</td><td class="num">${d.count}</td><td class="num">${fmt(d.valor)}</td><td class="num">${fmtPct(perdidos.length>0?d.count/perdidos.length*100:0)}</td></tr>`).join('')+
  ...
  if(pipeline.length){
    const stageGroups={};
    pipeline.forEach(r=>{const e=r.etapa||'(sem etapa)';if(!stageGroups[e])stageGroups[e]=[];stageGroups[e].push(r);});
    const stageNames=FUNIL_ORDER.filter(e=>stageGroups[e]).concat(Object.keys(stageGroups).filter(e=>!FUNIL_ORDER.includes(e)).sort());
    fpEl.innerHTML=stageNames.map(stage=>{
      const deals=stageGroups[stage].slice().sort((a,b)=>(b.prop||b.valor)-(a.prop||a.valor));
...
function renderSeller(){
  ...
  const allDataSellers=[...new Set(AppState.rawData.map(r=>r.consultor).filter(Boolean))];
  const extraSellers=allDataSellers.filter(s=>!metaSellers.includes(s)).sort();
  ...
  if(fixedEl){
    const curMK=`${CURRENT_YEAR}-${String(CURRENT_MONTH).padStart(2,'0')}`;
    const allRaw=AppState.rawData;
    const ytdG=allRaw.filter(r=>r.status==='ganho'&&r.monthKey&&r.monthKey.startsWith(String(CURRENT_YEAR)));
    const mesG=allRaw.filter(r=>r.status==='ganho'&&r.monthKey===curMK);
    const ytdP=allRaw.filter(r=>r.status==='perdido'&&r.monthKey&&r.monthKey.startsWith(String(CURRENT_YEAR)));
    const mesP=allRaw.filter(r=>r.status==='perdido'&&r.monthKey===curMK);
    ...
    const ytdConv=ytdG.length+ytdP.length>0?ytdG.length/(ytdG.length+ytdP.length)*100:0;
    const mesConv=mesG.length+mesP.length>0?mesG.length/(mesG.length+mesP.length)*100:0;
  ...
  sideEl.innerHTML=sellers.map((s,i)=>{
    const sg=ganhos.filter(r=>r.consultor===s);
    const v=sg.reduce((a,r)=>a+r.valor,0);
  ...
  mainEl.innerHTML=sellers.map((s,i)=>{
    const col=SELLER_COLORS[i%SELLER_COLORS.length];
    const pid='sp-panel-'+i;
    const sg=ganhos.filter(r=>r.consultor===s);
    const sp=perdidos.filter(r=>r.consultor===s);
    ...
    const m=(METAS_CONSULTOR[s]?.total||0)*numMonths;
    const pct=m>0?v/m*100:0,conv=sg.length+sp.length>0?sg.length/(sg.length+sp.length)*100:0;
    const ticket=sg.length>0?v/sg.length:0,falta=Math.max(0,m-v);
    const bizDays=businessDaysLeft();
    const dailyRate=bizDays>0?falta/bizDays:0;
    const empresa=AppState.rawData.filter(r=>r.consultor===s&&r.empresa).map(r=>r.empresa)[0]||'—';

    const alerts=[];
    if(pct<40)alerts.push({cls:'br',msg:`🔴 Meta em risco: apenas ${fmtPct(pct)} atingido`});
    else if(pct<70)alerts.push({cls:'bo',msg:`🟡 Atenção: ${fmtPct(pct)} da meta — ${bizDays} dias úteis restantes`});
    if(conv<20&&sg.length+sp.length>0)alerts.push({cls:'br',msg:`📉 Conversão baixa: ${fmtPct(conv)} (meta: 25%+)`});
    if(ticket>0&&ticket<2000)alerts.push({cls:'bo',msg:`💰 Ticket médio baixo: ${fmt(ticket)} — focar em LQ`});
    if(alerts.length===0)alerts.push({cls:'bg',msg:`✅ KPIs dentro do esperado para o período`});

    let plano='';
    if(pct<40)plano=`Recuperação urgente: ${falta>0?`fechar ${fmt(falta)} em ${bizDays} dias úteis — necessário ${fmt(dailyRate)}/dia útil.`:''} Priorizar os 3 maiores deals do pipeline.`;
    else if(pct<70)plano=`Acelerar fechamentos: contato diário com leads quentes. Focar em proposta + follow-up em 24h.`;
    else plano=`Manter ritmo. Prospectar novas contas LQ para garantir próximo mês.`;

    const allMonths12=['2026-01','2026-02','2026-03','2026-04','2026-05','2026-06','2026-07','2026-08','2026-09','2026-10','2026-11','2026-12'];
    const sgAll=AppState.rawData.filter(r=>r.status==='ganho'&&r.consultor===s);
    const monthVals=allMonths12.map(mk=>sgAll.filter(r=>r.monthKey===mk).reduce((a,r)=>a+r.valor,0));
    const chartId='chart-sp-'+i;
    const chartLabels=allMonths12.map(mk=>{const[,mo]=mk.split('-');return MONTH_NAMES[parseInt(mo)-1].substring(0,3);});
    ...
    const curMK=`${CURRENT_YEAR}-${String(CURRENT_MONTH).padStart(2,'0')}`;
    const spAll=AppState.rawData.filter(r=>r.status==='perdido'&&r.consultor===s);
    const sgAno=sgAll.filter(r=>r.monthKey&&r.monthKey.startsWith('2026'));
    const spAno=spAll.filter(r=>r.monthKey&&r.monthKey.startsWith('2026'));
    ...
    const convAno=sgAno.length+spAno.length>0?sgAno.length/(sgAno.length+spAno.length)*100:0;
    ...
    const convMes=sgCurMes.length+spCurMes.length>0?sgCurMes.length/(sgCurMes.length+spCurMes.length)*100:0;
    ...
    return `<div class="sp-panel${i===0?' active':''}" id="${pid}">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:12px">
          <div style="background:var(--bg3);border-radius:10px;padding:10px;border-top:2px solid var(--accent)">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px">
              ${[['REALIZADO',fmt(vAno),col],['META',fmt(mAno),'var(--warn)'],['ATINGIMENTO',fmtPct(pctAno),gaugeColor(pctAno)],['CONVERSÃO',fmtPct(convAno),'var(--text)']].map(([l,val,c])=>`
          ...
          <div style="background:var(--bg3);border-radius:10px;padding:10px;border-top:2px solid ${col}">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px">
              ${[['REALIZADO',fmt(vMes),col],['META',fmt(mMes),'var(--warn)'],['ATINGIMENTO',fmtPct(pctMes),gaugeColor(pctMes)],['CONVERSÃO',fmtPct(convMes),'var(--text)']].map(([l,val,c])=>`
              <div style="background:var(--bg2);border-radius:6px;padding:7px;text-align:center">
                <div style="font-family:'Poppins';font-size:8px;color:var(--text3)">${l}</div>
                <div style="font-family:'Poppins';font-size:14px;color:${c}">${val}</div>
              </div>`).join('')}
      ...
      <div class="card mb12">
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px">
          ${[
            ['Volume de Vendas', Math.min(100, pct).toFixed(0), gaugeColor(pct)],
            ['Taxa de Conversão', Math.min(100, conv*2).toFixed(0), gaugeColor(conv*2)],
            ['Ticket Médio', Math.min(100, ticket>0?(ticket/10000*100):0).toFixed(0), gaugeColor(ticket>0?(ticket/10000*100):0)]
          ].map(([nome,val,cor])=>`<div style="text-align:center;background:var(--bg3);border-radius:8px;padding:8px">
      ...
      <div class="card mb12">
        ...
        ${(()=>{
          const pipe=AppState.rawData.filter(r=>r.status==='pipeline'&&r.consultor===s).sort((a,b)=>(b.prop||b.valor)-(a.prop||a.valor)).slice(0,5);
          if(!pipe.length) return `<div style="color:var(--text3);font-size:11px;padding:8px">Nenhum lead em pipeline</div>`;
          const pipeTotProp=pipe.reduce((s,r)=>s+(r.prop||r.valor),0);
          return `<table class="dt"><thead><tr><th>Cliente</th><th>Etapa</th><th>Produto</th><th class="num">Proposta</th><th class="num">Dias</th></tr></thead><tbody>`+
            pipe.map(r=>`<tr><td class="hl">${(r.cliente||'—').substring(0,24)}</td><td>${r.etapa||'—'}</td><td><span class="badge bc">${r.produto}</span></td><td class="num">${fmt(r.prop||r.valor)}</td><td class="num">${r.dias||0}</td></tr>`).join('')+
      ...
      <div class="card">
        ...
        ${['RSS','Gerenciamento','Consultoria'].map(p=>{
          const pv=sg.filter(r=>r.produto===p).reduce((a,r)=>a+r.valor,0);
  ...
  spChartData.forEach(({chartId,col,monthVals,chartLabels})=>{
    ...
    if(!ctx)return;
    new Chart(ctx,{type:'line',data:{labels:chartLabels,datasets:[{label:'Vendas',data:monthVals,borderColor:col,backgroundColor:col+'22',tension:.3,fill:true,pointRadius:3,pointBackgroundColor:col}]},options:{...chartDefaults(),plugins:{...chartDefaults().plugins,tooltip:{callbacks:{label:c=>fmt(c.raw)}}}}});
  });
}
...
function renderGestor(){
  ...
  if(priorEl&&alavEl){
    ...
    const priorItems=Object.keys(METAS_CONSULTOR).map(s=>{
    ...
    }).filter(r=>r.pct<70).sort((a,b)=>b.falta-a.falta);

    priorEl.innerHTML=(priorItems.length?priorItems.map(r=>`
      ...
      </div>`).join(''):`<div class="alert-row green">✅ Todos os consultores acima de 70% da meta mensal</div>`);

    // Alavancas de Crescimento
    const totalPipe=mesPipe.reduce((s,r)=>s+(r.prop||r.valor),0);
    const topOrigens=Object.entries(mesG.reduce((acc,r)=>{acc[r.origem||'Não inf']=(acc[r.origem||'Não inf']||0)+r.valor;return acc;},{})).sort((a,b)=>b[1]-a[1]).slice(0,3);
    const fcPipe=mesPipe.filter(r=>{const e=(r.etapa||'').toLowerCase();return e.includes('forcast')||e.includes('forecast');});
    const fcVal2=fcPipe.reduce((s,r)=>s+(r.prop||r.valor),0);

    // Build per-seller pipeline analysis
    const sellersPipe=Object.keys(METAS_CONSULTOR).map(sl=>{
      const slPipe=mesPipe.filter(r=>r.consultor===sl);
      const slFc=slPipe.filter(r=>{const e=(r.etapa||'').toLowerCase();return e.includes('forcast')||e.includes('forecast');});
      ...
      return {sl,pct:slPct,falta:Math.max(0,slM-slV),fcVal:slFc.reduce((s,r)=>s+(r.prop||r.valor),0),pipeN:slPipe.length};
    ...
    alavEl.innerHTML=`
      ...
      ${sellersPipe.slice(0,3).map((r,i)=>{
        let acao='';
        if(r.pct<40)acao=`Ação imediata: reunião 1:1 hoje. Ligar para top ${r.pipeN} leads do pipeline.`;
        else if(r.pct<70)acao=`Focar nos ${r.fcVal>0?'deals em Forecast: '+fmt(r.fcVal):r.pipeN+' deals ativos'}. Follow-up em 24h.`;
  ...
  const months=monthsInRange('2026-01','2026-12');
  const numMonths=months.length;
  const allG=AppState.filteredData.filter(r=>r.status==='ganho'&&r.monthKey&&r.monthKey.startsWith('2026'));
  const allP=AppState.filteredData.filter(r=>r.status==='perdido'&&r.monthKey&&r.monthKey.startsWith('2026'));
  const endOfMonth=new Date(CURRENT_YEAR,CURRENT_MONTH,0);
  const daysLeft=Math.max(0,Math.ceil((endOfMonth-TODAY)/(1000*60*60*24)));

  const priority=Object.keys(METAS_CONSULTOR).map(s=>{
    const sg=allG.filter(r=>r.consultor===s),sp=allP.filter(r=>r.consultor===s);
    const v=sg.reduce((a,r)=>a+r.valor,0),m=METAS_CONSULTOR[s].total*numMonths;
    const pct=m>0?v/m*100:0,falta=Math.max(0,m-v);
    const conv=sg.length+sp.length>0?sg.length/(sg.length+sp.length)*100:0;
    const score=(100-pct)*2+(conv<20?50:0)+(falta/metaImpact(s)*100);
    return {s,pct,falta,conv,score,v,m};
  }).sort((a,b)=>b.score-a.score);

  paEl.innerHTML=`<div style="font-family:'Poppins';font-size:11px;color:var(--text3);margin-bottom:8px">Ordenado por impacto potencial no resultado do time (${daysLeft} dias restantes no mês)</div>`+
    priority.map((r,i)=>{
...
function metaImpact(s){return METAS_CONSULTOR[s]?METAS_CONSULTOR[s].total:1;}
...
function renderGestorPanel(){
  ...
  if(!selName){
    [execEl,kpiEl,insEl,pdcaEl].forEach(el=>{if(el)el.innerHTML='<div style="color:var(--text3);font-family:\'Poppins\';padding:12px">Selecione um consultor acima para ver a análise individual.</div>';});
    return;
  }
  ...
  const sp=fd.filter(r=>r.status==='perdido'&&r.consultor===selName);
  const pipe=fd.filter(r=>r.status==='pipeline'&&r.consultor===selName);
  ...
  const teamAvgTicket=fd.filter(r=>r.status==='ganho').length>0?fd.filter(r=>r.status==='ganho').reduce((a,r)=>a+r.valor,0)/fd.filter(r=>r.status==='ganho').length:1;
  const endOfMonth=new Date(CURRENT_YEAR,CURRENT_MONTH,0);
  const daysLeft=Math.max(0,Math.ceil((endOfMonth-TODAY)/(1000*60*60*24)));

  // [A5.2] Executive summary
  const fortes=[],criticos=[];
  if(pct>=70)fortes.push(`Meta em dia: ${fmtPct(pct)} atingido`);
  if(conv>=25)fortes.push(`Conversão sólida: ${fmtPct(conv)}`);
  if(ticket>=teamAvgTicket)fortes.push(`Ticket acima da média do time: ${fmt(ticket)}`);
  if(pct<50)criticos.push(`Meta crítica: apenas ${fmtPct(pct)} — falta ${fmt(falta)}`);
  else if(pct<70)criticos.push(`Meta em risco: ${fmtPct(pct)} — requer aceleração`);
  if(conv<20&&sg.length+sp.length>3)criticos.push(`Conversão baixa: ${fmtPct(conv)} (benchmark: 25%+)`);
  if(ticket<teamAvgTicket*0.7&&sg.length>0)criticos.push(`Ticket abaixo da média: ${fmt(ticket)} vs ${fmt(teamAvgTicket)} time`);
  if(diasMedio>30)criticos.push(`Ciclo de venda longo: média de ${diasMedio.toFixed(0)} dias no pipeline`);

  // Gestor dual-view: annual 2026 + current month
  const curMKGestor=`${CURRENT_YEAR}-${String(CURRENT_MONTH).padStart(2,'0')}`;
  const sgYTDGestor=AppState.rawData.filter(r=>r.status==='ganho'&&r.consultor===selName&&r.monthKey&&r.monthKey.startsWith('2026'));
  const spYTDGestor=AppState.rawData.filter(r=>r.status==='perdido'&&r.consultor===selName&&r.monthKey&&r.monthKey.startsWith('2026'));
  ...
  const convYTDGestor=sgYTDGestor.length+spYTDGestor.length>0?sgYTDGestor.length/(sgYTDGestor.length+spYTDGestor.length)*100:0;
  const sgMesGestor=AppState.rawData.filter(r=>r.status==='ganho'&&r.consultor===selName&&r.monthKey===curMKGestor);
  const spMesGestor=AppState.rawData.filter(r=>r.status==='perdido'&&r.consultor===selName&&r.monthKey===curMKGestor);
  ...
  const convMesGestor=sgMesGestor.length+spMesGestor.length>0?sgMesGestor.length/(sgMesGestor.length+spMesGestor.length)*100:0;
  ...
  const kpis=[
    {nome:'Receita Total 2026',atual:fmt(vYTDGestor),meta:fmt(mYTDGestor),devAbs:fmt(Math.abs(vYTDGestor-mYTDGestor)),devPct:fmtPct(mYTDGestor>0?Math.abs(vYTDGestor-mYTDGestor)/mYTDGestor*100:0),tendencia:pctYTDGestor>=70?'▲':'▼',comentario:`Anual 2026: ${fmtPct(pctYTDGestor)} atingido. Mês atual: ${fmtPct(pctMesGestor)} atingido.`},
    {nome:'Receita Mês Atual',atual:fmt(vMesGestor),meta:fmt(mMesGestor),devAbs:fmt(Math.abs(vMesGestor-mMesGestor)),devPct:fmtPct(mMesGestor>0?Math.abs(vMesGestor-mMesGestor)/mMesGestor*100:0),tendencia:pctMesGestor>=70?'▲':'▼',comentario:pctMesGestor<50?'Meta mensal crítica. Dobrar fechamentos restantes do mês.':pctMesGestor<70?'Ritmo mensal insuficiente. Focar nos deals maiores do pipeline.':'Ritmo mensal adequado. Manter consistência.'},
    {nome:'Taxa de Conversão',atual:fmtPct(conv),meta:'25%',devAbs:fmtPct(Math.abs(conv-25)),devPct:fmtPct(Math.abs(conv-25)/25*100),tendencia:conv>=25?'▲':'▼',comentario:conv<15?'Conversão crítica. Revisar qualificação de leads e script de fechamento.':conv<25?'Abaixo do benchmark. Implementar follow-up estruturado em 24h.':'Conversão saudável.'},
    {nome:'Ticket Médio',atual:fmt(ticket),meta:fmt(teamAvgTicket),devAbs:fmt(Math.abs(ticket-teamAvgTicket)),devPct:fmtPct(teamAvgTicket>0?Math.abs(ticket-teamAvgTicket)/teamAvgTicket*100:0),tendencia:ticket>=teamAvgTicket?'▲':'▼',comentario:ticket<teamAvgTicket*0.7?'Ticket baixo. Priorizar deals LQ e evitar SQ de baixo valor.':'Ticket dentro da faixa esperada.'},
    {nome:'Ciclo de Venda (dias)',atual:diasMedio.toFixed(0)+'d',meta:'30d',devAbs:Math.abs(diasMedio-30).toFixed(0)+'d',devPct:fmtPct(Math.abs(diasMedio-30)/30*100),tendencia:diasMedio<=30?'▲':'▼',comentario:diasMedio>45?'Ciclo muito longo. Identificar gargalos de negociação e definir prazo máximo por etapa.':diasMedio>30?'Ciclo acima do ideal. Pressionar fechamentos estagnados.':'Ciclo saudável.'},
    {nome:'Leads Trabalhados',atual:fmtN(all.length),meta:'—',devAbs:'—',devPct:'—',tendencia:'→',comentario:`${sg.length} ganhos, ${sp.length} perdidos, ${pipe.length} em pipeline.`}
  ];
  kpiEl.innerHTML=`<table class="dt"><thead><tr><th>KPI</th><th class="num">Atual</th><th class="num">Meta/Bench.</th><th class="num">Desvio</th><th>Tend.</th><th>Comentário</th></tr></thead><tbody>`+
    kpis.map(k=>`<tr>
  ...
  // Annual context
  if(pctYTDGestor<80){
    insights.push({titulo:`📊 Desempenho Anual 2026: ${fmtPct(pctYTDGestor)} da meta`,texto:`Acumulado 2026: ${fmt(vYTDGestor)} realizado de ${fmt(mYTDGestor)} meta (${CURRENT_MONTH} meses). ${pctYTDGestor>=70?'Ritmo anual adequado, mas requer atenção no mês atual.':'Ritmo anual abaixo do esperado. Necessário acelerar volume nos próximos meses para fechar o ano no verde.'}`});
  }
  if(pct<50){
    const addConv=(25-conv)/100*all.length*ticket;
    insights.push({titulo:`🎯 Prioridade #1: Recuperar meta de receita`,texto:`Faltam ${fmt(falta)} para atingir a meta do período (filtrado). Com ${daysLeft} dias restantes, é necessário fechar ${fmt(daysLeft>0?falta/daysLeft:falta)}/dia. Foco imediato nos ${pipe.length} deals de pipeline. Aumentar conversão de ${fmtPct(conv)} para 25% representaria +${fmt(addConv)} adicionais.`});
  }
  if(conv<20&&sg.length+sp.length>2){
    insights.push({titulo:`📊 Prioridade ${insights.length+1}: Aumentar Taxa de Conversão`,texto:`Taxa atual de ${fmtPct(conv)} está abaixo do benchmark (25%). Cada ponto percentual de melhoria representa aprox. ${fmt(ticket)} adicional de receita. Ação imediata: revisar etapas com maior churn no funil e implementar follow-up estruturado.`});
  }
  if(ticket<teamAvgTicket*0.8&&sg.length>1){
    insights.push({titulo:`💰 Oportunidade: Elevar Ticket Médio`,texto:`Ticket de ${fmt(ticket)} está ${fmtPct((1-ticket/teamAvgTicket)*100)} abaixo da média do time (${fmt(teamAvgTicket)}). Elevar para a média representaria +${fmt((teamAvgTicket-ticket)*sg.length)} no período. Focar em contas LQ e upsell de clientes existentes.`});
  }
  if(insights.length===0)insights.push({titulo:'✅ Performance dentro dos parâmetros',texto:`${selName} está atingindo os benchmarks principais. Foco em manutenção do ritmo e prospecção de novas contas para garantir o próximo mês.`});

  insEl.innerHTML=insights.map(ins=>`<div class="insight-item">
  ...
  // [A5.5] PDCA
  const pdcas=[];
  if(pct<70)pdcas.push({kpi:'Meta de Receita',col:'var(--bad)',
  ...
  if(conv<20&&sg.length+sp.length>2)pdcas.push({kpi:'Taxa de Conversão',col:'var(--warn)',
  ...
  if(pdcas.length===0)pdcas.push({kpi:'Crescimento e Consistência',col:'var(--ok)',
  ...
  pdcaEl.innerHTML=pdcas.map((p,i)=>`<div class="pdca-card">
    ...
    <div class="pdca-body" id="pdca-body-${i}">
      ${[['P','Planejar','var(--info)'],['D','Executar','var(--green3)'],['C','Verificar','var(--gold2)'],['A','Agir/Ajustar','var(--red2)']].map(([letra,nome,c],j)=>`
      <div class="pdca-cell">
...
function renderOrigem(){
  ...
  if(mesCardEl){
    if(no){mesCardEl.innerHTML='';}else{
      const mesG=allRaw.filter(r=>r.status==='ganho'&&r.monthKey===curMK);
      const mesV=mesG.reduce((s,r)=>s+r.valor,0);
      const mesAt=META_TIME_MES>0?mesV/META_TIME_MES*100:0;
      const topOri=Object.entries(mesG.reduce((acc,r)=>{acc[r.origem||'N/I']=(acc[r.origem||'N/I']||0)+r.valor;return acc;},{})).sort((a,b)=>b[1]-a[1]).slice(0,3);
  ...
  if(no){mesPerfEl.innerHTML=noDataHtml();
    const top10El=document.getElementById('origem-top10');if(top10El)top10El.innerHTML='';
    return;}

  // Performance por Origem — Mês Atual (uses unfiltered rawData for curMK)
  const mesGanhos=allRaw.filter(r=>r.status==='ganho'&&r.monthKey===curMK);
  const origens=[...new Set([...mesGanhos.map(r=>r.origem||'Não informada'),...METAS_ORIGEM.map(m=>m.origem)])].sort();
  const rows=origens.map(ori=>{
    ...
    return {ori,real,metaMes,pct,deals:og.length,ticket:og.length?real/og.length:0};
  }).sort((a,b)=>b.real-a.real);
  const rTotReal=rows.reduce((s,r)=>s+r.real,0),rTotMeta=rows.reduce((s,r)=>s+r.metaMes,0),rTotDeals=rows.reduce((s,r)=>s+r.deals,0);

  mesPerfEl.innerHTML=`<div style="font-family:'Poppins';font-size:10px;color:var(--text3);margin-bottom:6px">Mês atual: ${MONTH_NAMES[CURRENT_MONTH-1]}/${CURRENT_YEAR} · Não afetado por filtros</div>`+
  ...
  if(top10El){
    const mesDeals=allRaw.filter(r=>r.status==='ganho'&&r.monthKey===curMK).sort((a,b)=>b.valor-a.valor);
    if(mesDeals.length){
      // Group by origem, top 10 per group
      const byOri={};
      mesDeals.forEach(r=>{const o=r.origem||'Não informada';if(!byOri[o])byOri[o]=[];if(byOri[o].length<10)byOri[o].push(r);});
      const rows10=Object.entries(byOri).flatMap(([ori,deals])=>deals.map((r,i)=>({...r,_ori:ori,_rank:i+1})));
      top10El.innerHTML=`<div style="font-family:'Poppins';font-size:10px;color:var(--text3);margin-bottom:6px">Mês atual · Não afetado por filtros</div>`+
  ...
  const empresas=[{label:'RETEC S.A.',fn:e=>!isOeste(e),metaKey:'retec_sa'},{label:'RETEC Oeste',fn:isOeste,metaKey:'retec_oeste'}];
  quadEl.innerHTML=empresas.map(emp=>{
    const eg=ganhos.filter(r=>emp.fn(r.empresa));
    const eRows=origens.map(ori=>{
    ...
    }).filter(r=>r.real>0||r.metaTot>0).sort((a,b)=>b.real-a.real);
    const totalReal=eg.reduce((s,r)=>s+r.valor,0);
    const totalMeta=METAS_ORIGEM.reduce((s,m)=>(s+(m[emp.metaKey]||0)),0)*numMonths;
  ...
  cardsEl.innerHTML=rows.map(r=>kpiCard(r.ori,fmt(r.real),`Meta ${fmt(r.metaMes)} · ${fmtN(r.deals)} deal${r.deals!==1?'s':''}`,r.pct>=70?'c':r.pct>=40?'gold':'r',r.pct)).join('');

  /* Consolidado de Metas por Origem — YTD 2026 (unfiltered) */
  const atEl=document.getElementById('origem-ating-table');
  const ytdGanhos=allRaw.filter(r=>r.status==='ganho'&&r.monthKey&&r.monthKey.startsWith(String(CURRENT_YEAR)));
  const atRows=METAS_ORIGEM.map(m=>{
    const og=ytdGanhos.filter(r=>(r.origem||'Não informada')===m.origem);
    const real=og.reduce((s,r)=>s+r.valor,0);
  ...
  const atTotReal=atRows.reduce((s,r)=>s+r.real,0),atTotMeta=atRows.reduce((s,r)=>s+r.metaAcum,0);
  atEl.innerHTML=`<div style="font-family:'Poppins';font-size:10px;color:var(--text3);margin-bottom:6px">Acumulado 2026 (Jan–${MONTH_NAMES[CURRENT_MONTH-1]}) · Não afetado por filtros</div>`+
    `<table class="dt"><thead><tr>
  ...
  </tr></thead><tbody>${atRows.map(r=>`<tr>
    <td>${r.ori}</td>
...
function renderPorte(){
  ...
  if(no){kpisEl.innerHTML=noDataHtml();document.getElementById('porte-benchmark').innerHTML='';
    document.getElementById('porte-consultor-table').innerHTML='';document.getElementById('porte-conv-table').innerHTML='';return;}
  ...
  const sqTicket=sqG.length?sqG.reduce((s,r)=>s+r.valor,0)/sqG.length:0;
  ...
  const convLQ=lqAll.length>0?lqAll.filter(r=>r.status==='ganho').length/lqAll.length*100:0;
  const convSQ=sqAll.length>0?sqAll.filter(r=>r.status==='ganho').length/sqAll.length*100:0;

  /* KPI cards */
  kpisEl.innerHTML=[
    kpiCard('Receita LQ',fmt(lqVal),`${fmtN(lqG.length)} deals · ${fmtPct(lqPctRec)} da receita · Bench: 70%`,'c',lqPctRec),
    kpiCard('Receita SQ',fmt(sqVal),`${fmtN(sqG.length)} deals · ${fmtPct(sqPctRec)} da receita · Bench: 30%`,'',sqPctRec),
  ...
  function benchCard(title,real,bench,label,color){
    ...
    const diffSign=diff>=0?`+${fmtPct(Math.abs(diff))}`:`-${fmtPct(Math.abs(diff))}`;
    const diffColor=diff>=0?'var(--ok)':'var(--bad)';
    const barW=Math.min(100,real);const benchW=Math.min(100,bench);
    return `<div class="card">
  ...
  bmEl.innerHTML=[
    benchCard('Distribuição por Quantidade — LQ',lqPctQtd,20,'Benchmark: 20% LQ / 80% SQ (qtd)','var(--info)')+
    benchCard('Distribuição por Quantidade — SQ',sqPctQtd,80,'Benchmark: 80% SQ / 20% LQ (qtd)','var(--blue)'),
    benchCard('Distribuição por Receita — LQ',lqPctRec,70,'Benchmark: 70% LQ / 30% SQ (receita)','var(--info)')+
    benchCard('Distribuição por Receita — SQ',sqPctRec,30,'Benchmark: 30% SQ / 70% LQ (receita)','var(--blue)')
  ].map((pair,i)=>`<div class="card"><div class="card-title">${i===0?'🔢 Benchmark Quantidade':'💰 Benchmark Receita'}</div>
    <div class="g2">${pair}</div></div>`).join('');
  ...
  const cRows=sellers.map(sel=>{
    ...
    const lPctRec=sv>0?lv/sv*100:0;const qPctRec=sv>0?qv/sv*100:0;
    const lPctQtd=sg.length>0?slq.length/sg.length*100:0;const qPctQtd=sg.length>0?ssq.length/sg.length*100:0;
    return {sel,total:sg.length,lqN:slq.length,sqN:ssq.length,lv,qv,sv,lPctRec,qPctRec,lPctQtd,qPctQtd};
  }).sort((a,b)=>b.sv-a.sv);
  const ptTotLQ=cRows.reduce((s,r)=>s+r.lqN,0),ptTotSQ=cRows.reduce((s,r)=>s+r.sqN,0);
  const ptTotLV=cRows.reduce((s,r)=>s+r.lv,0),ptTotQV=cRows.reduce((s,r)=>s+r.qv,0);
  const ptTot=ptTotLQ+ptTotSQ,ptTotV=ptTotLV+ptTotQV;
  consEl.innerHTML=cRows.length?`<table class="dt"><thead><tr>
  ...
  </tbody></table>`:noDataHtml('Sem dados de negócios ganhos no período');
  ...
  const mRows=months.map(mk=>{
    ...
    const [y,m]=mk.split('-');
    return {label:MONTH_NAMES[+m-1]+'/'+y.slice(2),lqN:mlq.length,sqN:msq.length,lv,qv,mv,lPctRec,conv};
  });
  const cvTotLQ=mRows.reduce((s,r)=>s+r.lqN,0),cvTotSQ=mRows.reduce((s,r)=>s+r.sqN,0);
  const cvTotLV=mRows.reduce((s,r)=>s+r.lv,0),cvTotQV=mRows.reduce((s,r)=>s+r.qv,0),cvTotMV=mRows.reduce((s,r)=>s+r.mv,0);
...
function renderTipo(){
  ...
  if(no){kpisEl.innerHTML=noDataHtml();document.getElementById('tipo-table').innerHTML='';
    document.getElementById('tipo-consultor-table').innerHTML='';return;}
  ...
  const pRows=PRODUTOS.map(prod=>{
    ...
    const metaMes=Object.values(METAS_CONSULTOR).reduce((s,m)=>s+(m[prod]||0),0);
    const meta=metaMes*numMonths;
    const pct=meta>0?real/meta*100:0;
    const conv=(pg.length+perd.length)>0?pg.length/(pg.length+perd.length)*100:0;
    const ticket=pg.length?real/pg.length:0;
    return {prod,real,meta,pct,deals:pg.length,conv,ticket};
  });

  /* KPI cards per tipo */
  kpisEl.innerHTML=pRows.map(r=>kpiCard(r.prod,fmt(r.real),
    `Meta ${fmt(r.meta)} · ${fmtPct(r.pct)} · ${fmtN(r.deals)} deals`,
    r.pct>=70?'c':r.pct>=40?'gold':'r',r.pct)).join('');

  /* Tabela por tipo */
  const tipoEl=document.getElementById('tipo-table');
  const tTotReal=pRows.reduce((s,r)=>s+r.real,0),tTotMeta=pRows.reduce((s,r)=>s+r.meta,0),tTotDeals=pRows.reduce((s,r)=>s+r.deals,0);
  ...
  if(cEl){
    const colors={'RSS':'#bcd6ff','Gerenciamento':'#bcd6ff','Consultoria':'#d9c8ff'};
    const fixedMonths=monthsInRange(`${CURRENT_YEAR}-01`,`${CURRENT_YEAR}-${String(CURRENT_MONTH).padStart(2,'0')}`);
    const allGanhos2026=AppState.rawData.filter(r=>r.status==='ganho'&&r.monthKey&&r.monthKey.startsWith(String(CURRENT_YEAR)));
    const datasets=PRODUTOS.map(prod=>({
      label:prod,
      data:fixedMonths.map(mk=>allGanhos2026.filter(r=>r.produto===prod&&r.monthKey===mk).reduce((s,r)=>s+r.valor,0)),
      borderColor:colors[prod],backgroundColor:colors[prod]+'22',fill:true,tension:.3,pointRadius:3
    }));
    new Chart(cEl,{type:'line',data:{labels:fixedMonths.map(mk=>{const[y,m]=mk.split('-');return MONTH_NAMES[+m-1].substring(0,3)+'/'+y.slice(2);}),datasets},
      options:{...chartDefaults(),plugins:{...chartDefaults().plugins,
  ...
  const sellers=Object.keys(METAS_CONSULTOR);
  const colHeaders=PRODUTOS.map(p=>`<th class="num" colspan="2">${p}</th>`).join('');
  const colSubs=PRODUTOS.map(()=>`<th class="num">R$</th><th class="num">%Meta</th>`).join('');
  const cRows=sellers.map(sel=>{
    const cols=PRODUTOS.map(prod=>{
      const sg=ganhos.filter(r=>r.consultor===sel&&r.produto===prod);
  ...
  const totCols=PRODUTOS.map((_,pi)=>({real:cRows.reduce((s,r)=>s+r.cols[pi].real,0),meta:cRows.reduce((s,r)=>s+r.cols[pi].meta,0)}));
...
function renderEvolutivo(){
  ...
  if(no){cardsEl.innerHTML=noDataHtml();document.getElementById('evo-table').innerHTML='';
    dc('chart-evo-main');return;}

  /* Group by year → month → company */
  const years=[...new Set(allData.filter(r=>r.status==='ganho'&&r.data).map(r=>r.data.substring(0,4)))].sort();
  ...
  if(cEl&&years.length){
    const yearColors=['rgba(255,255,255,.55)','#bcd6ff','#bcd6ff','#9effd4','#ffd79e','#d9c8ff'];
    const datasets=years.map((yr,i)=>{
      const data=months12.map(m=>ganhos.filter(r=>r.monthKey===`${yr}-${m}`).reduce((s,r)=>s+r.valor,0));
      const col=yearColors[i%yearColors.length];
      const isCurrentYear=yr===String(CURRENT_YEAR);
      return {label:yr,data,borderColor:col,backgroundColor:col+'22',
        tension:.3,fill:isCurrentYear,pointRadius:isCurrentYear?4:2,
        borderWidth:isCurrentYear?2.5:1.5,borderDash:isCurrentYear?[]:[4,3]};
    });
  ...
  cardsEl.innerHTML=evoYears.map((yr,i)=>{
    const yg=ganhos.filter(r=>r.data&&r.data.startsWith(yr));
    const vTotal=yg.reduce((s,r)=>s+r.valor,0);
    const byMonth=months12.map(m=>yg.filter(r=>r.data&&r.data.substring(5,7)===m).reduce((s,r)=>s+r.valor,0));
    ...
    const crescimento=pvTotal>0?(vTotal-pvTotal)/pvTotal*100:0;
    const mese=months12.filter(m=>{const mk=`${yr}-${m}`;return new Date(mk+'-01')<=TODAY;}).length||1;
    const proj=vTotal/mese*12;
    return kpiCard(`${yr}`,fmt(vTotal),
  ...
  const evoTotals=evoYears.map(yr=>{
    const yg=ganhos.filter(r=>r.data&&r.data.startsWith(yr));
    const sa=yg.filter(r=>!isOeste(r.empresa)).reduce((s,r)=>s+r.valor,0);
    const oeste=yg.filter(r=>isOeste(r.empresa)).reduce((s,r)=>s+r.valor,0);
  ...
  </tr></thead><tbody>${months12.map((m,mi)=>{
    const cells=evoYears.map(yr=>{
      const mk=`${yr}-${m}`;
...
function exportSeller(idx){
  ...
  if(!card){alert('Painel não encontrado. Selecione o consultor primeiro.');return;}
  if(typeof html2canvas==='undefined'){alert('html2canvas não disponível.');return;}
  html2canvas(card,{
    ...
    a.href=url;a.download=`gvc_seller_${sellerName.replace(/\s+/g,'_')}_${new Date().toISOString().slice(0,10)}.png`;
